...
usuarioCtrl.getUsu = async (req, res) => {
  ...
  try {
    ...
    const usuariosConFoto = usuarios.map((usuario) => ({
      ...usuario._doc,
      foto: usuario.foto
        ? `https://backend-mern-yr4j.onrender.com/img/${usuario.foto}`
        : `https://backend-mern-yr4j.onrender.com/img/noFoto.png`, // Imagen por defecto si no hay foto
    }));
  ...
  try {
    ...
    if (!nombre || !correo || !apellido || !telefono) {
      return res
        .status(400)
        .json({
          message: "El nombre, apellidos, teléfono y correo son obligatorios",
        });
    }
    ...
    res
      .status(201)
      .json({ message: "Usuario creado", usuario: usuarioGuardado });
  } catch (error) {
    ...
    if (usuario.foto && usuario.foto !== "noFoto.png") {
      ...
      fs.unlink(filePath, (err) => {
        if (err) {
          ...
          return res
            .status(500)
            .json({ message: "Error al eliminar la foto", error: err.message });
        }
      });
    }
      ...
      fs.unlink(filePath, (err) => {
        if (err) {
          ...
          return res
            .status(500)
            .json({
              message: "Error al eliminar la foto antigua",
              error: err.message,
            });
        }
      });
